...
const buildGraph = function (config) {
  ...
  const p = new Promise((resolve, reject) => {
    const version = execSync(`docker pull opentransport/opentripplanner:${graphBuildTag};docker run --rm --entrypoint /bin/bash opentransport/opentripplanner:${graphBuildTag}  -c "java -jar otp-shaded.jar --version"`)
    const commit = version.toString().match(/commit: ([0-9a-f]+)/)[1]

    const buildGraph = exec(`docker run -v ${hostDataDir}/build:/opt/opentripplanner/graphs --rm --entrypoint /bin/bash opentransport/opentripplanner:${graphBuildTag}  -c "java -Xmx10g -jar otp-shaded.jar --build graphs/${config.id}/router"`, { maxBuffer: constants.BUFFER_SIZE })
